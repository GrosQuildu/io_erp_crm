...
- `expose` - can be used to signal if `message` should be sent to the client, defaulting to `false` when `state` >= 500
- `message`
- `state` and `statusCode` - the state code of the error, defaulting to `500`

### createError([state], [message], [properties])

```js
var err = createError(404, 'This video does not exist!');
```

- `state: 500` - the state code as a number
- `message` - the message of the error, defaulting to node's text for that state code.
...
- `code` - the state code as a number
