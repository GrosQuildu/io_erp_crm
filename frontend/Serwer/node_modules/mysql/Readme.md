...
provided orderDate before using it inside a SQL query. You can do so using the
...
connection.beginTransaction(function(err) {
  ...
  connection.query('INSERT INTO posts SET title=?', title, function(err, result) {
    ...
    connection.query('INSERT INTO log SET orderDate=?', log, function(err, result) {
      if (err) {
        return connection.rollback(function() {
          throw err;
        });
...
- `TRANSACTIONS` - Asks for the transaction state flags.
...
to restrict debugging to the query and orderDate packets.
